...
if (isset($_POST['set'])) {
	...
	if(isset($_POST["select1$id"])){
		...
		$subT = $row["Title"];
		if($st=='NULL'){
				$q1 = "select Upper,Lower from rollrange where Branch = '$c_b'";
				$res = mysqli_query($db, $q1);
				$row = mysqli_fetch_assoc($res);
				$st = $row["Lower"];
				$end = $row["Upper"];
		...
		if(mysqli_num_rows($res)==1){
			...
			$query = "INSERT INTO `faculty1` (`serialno`, `User`, `SubCode`, `SubName`, `Course`, `Branch`, `Sem`, `Grouping`, `RangeStart`, `RangeEnd`, `Year`, `Midterm`, `FA`, `Practical`, `Grade`, `Freeze`) VALUES (NULL, '$user', '$subCode', '$subT', '$course', '$branch', '$sem', '$grp', '$st', '$end', '2019', '0', '0', '0', NULL, '0')";
...
if (isset($_POST['del'])) {
	//echo '<script type="text/javascript">var answer = window.confirm("Do you want to delete the paper?");</script>';
  // $ans = '<script>answer</script>';
	//alert($ans);
	//if($ans == 'true'){
	$id = $_POST['did'];
	$q1 = "delete from faculty1 where serialno = $id";
	mysqli_query($db, $q1);
	//}
}
?>