...
<div id="image">
    ...
    <div id="navbar">
        <ul>
			...
			<li><a href="attend.php">Generate Attendance</a></li>
	...
	<div id="maincontent">
	<form method="post" action ="attsheet.php">
	<table cellpadding=25px >
			<tr><td>Programme <select class='form-control' name='select1' id="select1" required>
			...
			<td>Semester <select class='form-control' name='select3' id="select3" required>
			<option value='1' selected>1</option>
			<option value='2'>2</option></select></td>

			<td>Subject <select class='form-control' name='select2' id="select2" required>
			<option value=''>Please select from below</option>
      ...
      $("#select1").data('options', $('#select2 option').clone());
          ...
          var id = $("#select1").val() + ' ' + $("#select3").val();
          var options = $("#select1").data('options').filter('[name="' + id + '"]');
          $("#select2").html(options);
        });
    });
